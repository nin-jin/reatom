...
import { futureFrom, futureCombine } from '../src'
...
;() => {
  ...
  ;(async () => {
    ...
    const f0 = futureFrom(f0Map)
    const f1 = f0.chain(f1Map)
    const f2 = futureCombine([f0, f1]).chain(f2Map)
  ...
  ;(async () => {
    ...
    const f3 = futureFrom((v: number) => Promise.resolve(`_${v}`)).chainAsync(
      track,
    )
...
;(() => {
  // return
  const entry = futureFrom() //1
  const a = entry.chainAtom(0, (v, s) => s + 1) //2
  const b = a.chain((a) => a + 1) //3
  const c = a.chain((a) => a + 1) //4
  const d = futureCombine([b, c]).chain(([b, c]) => b + c) //5
  const e = d.chain((d) => d + 1) //6
  const f = d.chain((d) => d + 1) //7
  ...
  while (i--) {
    ...
    // if (!(i % 50)) {
    //   const future = futureCombine([e, f]).chain(([e, f]) => e + f)
